...
const header = () => {
    return `
        <header id='header'>
            <p id='logo'>New York Times Best Sellers</p>
            <p id='menuIcon' class="material-symbols-outlined">menu</p>
        </header>
    `
}
...
const BookCards = (obj) => {
    return obj.cards.map((b,i) => bookCard(i+1, b.title, b.sub, b.text)).join(' ')
}
...
const loadEvent = () => {
    ...
    rootElement.insertAdjacentHTML('beforeend', bookCardsBox)

    const bookCardsBoxElement = document.getElementById('bookCards')
    bookCardsBoxElement.insertAdjacentHTML('beforeend', BookCards(books))
